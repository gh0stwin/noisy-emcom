...
class ReinforceSpeakerLoss(SpeakerLoss):
    ...
    def compute_speaker_loss(
        ...
        action_log_prob = jnp.transpose(games.speaker_outputs.action_log_prob, [1, 0])
        ...
        # Transforms to [T, B, F] receives [B, F, T].
        speaker_policy_logits = jnp.transpose(
            games.speaker_outputs.policy_logits, [2, 0, 1]
        )
        ...
        kl_target_loss = (
            rlax.softmax().kl(speaker_policy_logits, target_speaker_policy_logits)
            * self._speaker_kl_target
        )
        ...
        stats = dict(
            speaker_value=value_stats,
            speaker_value_loss=value_loss,
            speaker_speaker_loss=speaker_loss,
            speaker_policy_loss=policy_loss,
            speaker_entropy_loss=entropy_loss,
            speaker_kl_target_loss=kl_target_loss,
...
class PolicyGradientSpeakerLoss(SpeakerLoss):
    ...
    def __init__(self, speaker_entropy: float, use_baseline: bool = False) -> None:
        self._speaker_entropy = speaker_entropy
        self._use_baseline = use_baseline
...
class DummySpeakerLoss(SpeakerLoss):
    ...
    def compute_speaker_loss(
        ...
        """Computes the policy gradient loss."""
        del games, reward
        return types.SpeakerLossOutputs(loss=0.0, stats={})
